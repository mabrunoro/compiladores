...
void set_type(SymTable* st, int i, Type type) {
    st->t[i].type = type;
}
...
Type get_type(SymTable* st, int i) {
    return st->t[i].type;
}

void remove_last(SymTable* st) {
    st->size--;
}
...
void print_sym_table(SymTable* st) {
    ...
    for (int i = 0; i < st->size; i++) {
         printf("Entry %d -- name: %s, line: %d, type %s\n", i,
                get_name(st, i), get_line(st, i), TYPE_STRING[get_type(st, i)]);
    }
}
