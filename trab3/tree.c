...
void printnode(tree *node, int level)
{
	if(node == NULL)
		return;
	printf("%d: Node -- Tipo: %d -- Count: %d -- Val: %d\n", level, node->item, node->cnt, node->tval);
	tree* n;
...
{
	int my_nr = nr++;
	if((node->item == IDN) || (node->item == STRINGN))
		printf("node%d[label=\"%s,%d\"];\n", my_nr, getenumname(node->item,node->tval),node->tval);
	else
		printf("node%d[label=\"%s\"];\n", my_nr, getenumname(node->item,node->tval));
	tree *n;
	for(n = node->child; n != NULL; n = n->brother)
	{
